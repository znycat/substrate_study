...
const StorageKey_js_1 = require("../../primitive/StorageKey.js");
    ...
    describe('storage with default values', () => {
        ...
        pallets.filter(({ storage }) => storage.isSome).forEach(({ name, storage }) => {
            ...
            storage.unwrap().items.forEach(({ fallback, modifier, name, type }) => {
                const inner = (0, StorageKey_js_1.unwrapStorageType)(registry, type, modifier.isOptional);
                const location = `${sectionName}.${(0, util_1.stringCamelCase)(name)}: ${inner}`;
                it(location, () => {
                    expect(() => {
                        try {
                            const instance = registry.createTypeUnsafe(registry.createLookupType((0, StorageKey_js_1.unwrapStorageSi)(type)), [(0, util_1.hexToU8a)(fallback.toHex())], { isOptional: modifier.isOptional });
